...
void VOLIRO_CTRL::camera_cb(geometry_msgs::Pose pose){
    ...
    _T_tag = _T_drone*_T_cb*_T_temp_cb*_T_tag;
...
void VOLIRO_CTRL::feedback(){
    ...
    vel_temp = utilities::Ad_f((_T_cb*_T_temp_cb).inverse())*_local_vel_6d;
...
void::VOLIRO_CTRL::ibvs(){
    ...
    sd << -0.112293,  -0.25855, 0.118281, -0.256641,  0.126541, -0.509248,  -0.11796, -0.511741;
    ...
    F_des = utilities::QuatToMat(_local_quat).transpose()*(-_mass*g+_mass*utilities::Ad_f(_T_cb*_T_temp_cb)*vs).block<3,1>(3,0);
    _F_des = F_des;
    M_des =  utilities::QuatToMat(_local_quat).transpose()*((NL_comp)+(J*((utilities::Ad_f(_T_cb*_T_temp_cb)*vs).block<3,1>(0,0))));
    _M_des = M_des;
